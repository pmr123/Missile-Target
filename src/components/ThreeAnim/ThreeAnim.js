	...
	componentDidMount() {
		...
		function place_interceptors(antiballistic_data) {
			...
			gltfloader.load('models/interceptor/scene.gltf', (gltf) => {
				...
				interceptor.position.set(Number(single_antiballistic.x) * 10, Number(single_antiballistic.y) * 10-60, Number(single_antiballistic.z) * 10);
		...
		function animate_ballistic() {
			...
			if (missiles_obj.position.y >= antimissiles_obj.position.y )//|| missiles_obj.position.z != antimissiles_obj.position.z || missiles_obj.position.x != antimissiles_obj.position.x) {
			{	time = time + 0.01;
